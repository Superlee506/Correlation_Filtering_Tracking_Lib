...
namespace cf_tracking
{
    ...
    public:
                    ...
                    const Size& targetSize, const T& currentScaleFactor)
        {
            ...
            _scaleModelSz.height = static_cast<T>(std::max(_SCALE_CELL_SIZE + 1, int(_scaleModelSz.height)));
            _scaleModelSz.width = static_cast<T>(std::max(_SCALE_CELL_SIZE + 1, int(_scaleModelSz.width)));

            // expand ysf to have the number of rows of scale samples
//            Size tmp_feature_size;
//            tmp_feature_size.width = std::max(1, int(floor(_scaleModelSz.width / _SCALE_CELL_SIZE)));
//            tmp_feature_size.height = std::max(1, int(floor(_scaleModelSz.height / _SCALE_CELL_SIZE)));
//            int ysfRow = static_cast<int>(tmp_feature_size.width * tmp_feature_size.height * FFC::numberOfChannels());
            ...
            if (getScaleTrainingData(image, pos,
                ...
                return false;
            // 初始化最大最小的scale
            cv::Size imszie = image.size();
            _MIN_SCALE_FACTOR = std::pow(_SCALE_STEP, ceil(log(std::max(5.0/_targetSize.width, 5.0/_targetSize.height))/log(_SCALE_STEP)));
            _MAX_SCALE_FACTOR = std::pow(_SCALE_STEP, floor(log(std::min(imszie.width/(float)_targetSize.width, imszie.height/(float)_targetSize.height))/log(_SCALE_STEP)));
    ...
    private:
                              ...
                              cv::Mat& features, T scale) const
        {
            ...
            int idxScale = 1;
            //std::cout<<_scaleFactors<<std::endl;
            //std::cout<<_scaleWindow<<std::endl;
